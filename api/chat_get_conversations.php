...
// Get all conversations with user info - join by both id and phone
$sql = "SELECT c.*, u.fullname, u.email, u.phone 
        FROM chat_conversations c 
        LEFT JOIN users u ON c.user_id = u.id OR c.user_id = u.phone
...
while ($row = $result->fetch_assoc()) {
    ...
    $msg_stmt->bind_param("s", $row['user_id']);
    $msg_stmt->execute();

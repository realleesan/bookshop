...
// Lấy số lượng người dùng (bao gồm cả admin và user)
...
function getTotalRevenue() {
  ...
  return orders.reduce((sum, order) => {
    const value = Number(order.tongtien);
...
// Hiển thị thống kê lên giao diện
function renderStatistics() {
  document.getElementById("amount-user").textContent = getAmountUser();
  document.getElementById("amount-product").textContent = getAmountProduct();
  document.getElementById("doanh-thu").textContent =
    formatVND(getTotalRevenue());
}

// Gọi khi load trang
window.addEventListener("load", renderStatistics);
...
function loadFooterDiscount() {
  ...
  fetch("./api/coupon.php", {
    ...
    .catch((error) => {
      console.error("Error loading coupon duration:", error);
    });
}
    ...
    .then((data) => {
      ...
      if (data.success) {
        ...
        fetch("./api/coupon.php", {
          ...
          body: "action=update_coupon_duration&hours=" + hours + "&minutes=" + minutes + "&seconds=" + seconds,
        })
          .then((response) => response.json())
          .then((durationData) => {
            if (durationData.success) {
              ...
              messageEl.textContent = durationData.message || "Lỗi cập nhật thời hạn!";
              messageEl.style.color = "red";
              toast({
                title: "Lỗi",
                message: durationData.message || "Không thể cập nhật thời hạn!",
                type: "error",
                duration: 3000,
              });
            }
          ...
          .catch((error) => {
            console.error("Error saving coupon duration:", error);

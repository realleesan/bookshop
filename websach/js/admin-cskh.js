...
async function loadConversations() {
  try {
    ...
    if (!conversations || conversations.length === 0) {
      container.innerHTML =
        '<div class="cskh-empty">Chưa có cuộc trò chuyện nào</div>';
      return;
    }
...
async function loadMessages(userId) {
  try {
    ...
    if (!messages || messages.length === 0) {
      container.innerHTML =
        '<div class="cskh-empty">Chưa có tin nhắn nào</div>';
      return;
    }
...
async function deleteConversation() {
  ...
  try {
    ...
    if (result.success) {
      ...
      document.getElementById("cskh-messages").innerHTML =
...
function startCSKHPolling() {
  ...
  cskhInterval = setInterval(() => {
    ...
    // Kiểm tra xem tab CSKH có đang mở không
    if (cskhSection && !cskhSection.classList.contains("hidden")) {
      // Luôn load danh sách bên trái để cập nhật số tin nhắn chưa đọc (badge)
      loadConversations();

      // Nếu đang chọn một người chat cụ thể
      if (selectedUserId) {
        const container = document.getElementById("cskh-messages");

        // KIỂM TRA: Nếu bạn đang kéo lên xem tin cũ (cách đáy > 150px)
        // thì KHÔNG load tin nhắn mới để giữ nguyên vị trí đang xem.
        const isReadingOld =
        ...
        if (!isReadingOld) {
          ...
          console.log("Bạn đang xem tin nhắn cũ, hệ thống sẽ không tự nhảy.");
        }
      }
    }
  }, 600000);
  ...
  try {
    ...
    if (cskhItem) {
      ...
      if (!badge) {
        ...
        badge.style.cssText =
          "background: #dc3545; color: white; font-size: 11px; padding: 2px 6px; border-radius: 10px; margin-left: 5px;";
        cskhItem
          .querySelector(".sidebar-link")
          .insertAdjacentElement("beforeend", badge);
      }

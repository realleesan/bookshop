...
btnUpdateProductIn.addEventListener("click", async (e) => {
  ...
  if (!products || !Array.isArray(products)) {
    toast({
      title: "Lỗi",
      message: "Danh sách sản phẩm không tìm thấy hoặc không hợp lệ!",
      type: "error",
      duration: 3000,
    });
    return;
  ...
  if (indexCur === undefined || indexCur < 0 || indexCur >= products.length) {
    toast({
      title: "Lỗi",
      message: "Chỉ số sản phẩm không hợp lệ!",
      type: "error",
      duration: 3000,
    });
    return;
  ...
  if (file) {
    ...
    document.querySelector(".upload-image-preview").src !== imgProduct &&
    !document.querySelector(".upload-image-preview").src.startsWith("data:")
  ) {
    // Image preview changed but no new file - it's a path change
    newImgPath =
      getPathImage(document.querySelector(".upload-image-preview").src) ||
      imgProduct;
  }

  if (
    newImgPath !== imgProduct ||
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          title: "Lỗi",
          message: result.message || "Có lỗi xảy ra khi cập nhật sản phẩm!",
          type: "error",
          duration: 3000,
        });
      }
...
function changeStatus(id, newStatus) {
  ...
  fetch("update_order_status.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        toast({
          title: "Thất bại",
          message:
            data.message || "Đã xảy ra lỗi khi cập nhật trạng thái đơn hàng!",
          type: "error",
          duration: 3000,
        });
      }
    ...
    .catch((error) => {
      ...
      toast({
        title: "Thất bại",
        message: "Đã xảy ra lỗi kết nối: " + error.message,
        type: "error",
        duration: 3000,
      });
    });
...
function getStatusClassUser(trangthai) {
  switch (parseInt(trangthai)) {
    case 0:
      return "status-no-complete"; // Chờ xử lý - màu vàng
    case 1:
      return "status-processing"; // Đang xử lý - màu xanh dương
    case 2:
      return "status-complete"; // Đã xử lý - màu xanh lá
    case 3:
      return "status-cancelled"; // Đã hủy - màu đỏ
    default:
      return "status-no-complete";
  }
}
...
function getStatusClass(trangthai) {
  switch (parseInt(trangthai)) {
    case 0:
      return "status-pending"; // Chờ xử lý - màu vàng
    case 1:
      return "status-processing"; // Đang xử lý - màu xanh dương
    case 2:
      return "status-complete"; // Đã xử lý - màu xanh lá
    ...
    fetch("delete_order.php", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          toast({
            title: "Success",
            message: data.message || "Xóa đơn hàng thành công!",
            type: "success",
            duration: 3000,
          });
        } else {
          toast({
            title: "Thất bại",
            message: data.message || "Đã xảy ra lỗi khi xóa đơn hàng!",
            type: "error",
            duration: 3000,
          });
        }
...
function detailOrder(id) {
  ...
  ctDon.forEach((item) => {
    ...
    let detaiSP = products.find(
      (product) =>
        String(product.id) === String(item.id) || product.id == item.id,
    );
...
function showUserArr(arr) {
  ...
  if (arr.length == 0) {
    ...
    arr.forEach((account, index) => {
      ...
      let vaitro =
        account.userType == 1
          ? `<span class="status-admin">Quản trị</span>`
          : `<span class="status-user">Người dùng</span>`;
      accountHtml += ` <tr>
            <td>${index + 1}</td>
...
function deleteAccount(phone) {
  ...
  console.log(
    "deleteAccount called with phone:",
    phone,
    "| index found:",
    index,
  );
    ...
    fetch("delete_account.php", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          toast({
            title: "Thất bại",
            message: data.message || "Đã xảy ra lỗi khi xóa tài khoản!",
            type: "error",
            duration: 3000,
          });
        }
...
function editAccount(phone) {
  ...
  const isActive =
    account.status == 1 || account.status === "1" || account.status === true;
...
function updateAccountFunc(e) {
  ...
  let index = accounts.findIndex(
    (item) => String(item.phone) === String(window.selectedAccountPhone),
  );
  if (index !== -1) {
  ...
  fetch("update_account.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        let accounts = JSON.parse(localStorage.getItem("accounts"));
        let index = accounts.findIndex(
          (item) => String(item.phone) === String(phone),
        );
        if (index !== -1) {
          accounts[index].fullname = fullname;
          accounts[index].phone = phone;
...
function displayRatings(ratings) {
  ...
  ratings.forEach((rating, index) => {
    ...
    const productTitle =
      rating.product_title || "Sản phẩm #" + rating.product_id;
    const userName =
      rating.display_name || rating.fullname || rating.email || "Ẩn danh";
...
// Load footer discount percentage and duration from server
function loadFooterDiscount() {
  alert("DEBUG: loadFooterDiscount() called!");

  // Load duration settings
  const durationUrl = "./api/coupon.php";
  alert("DEBUG: Fetching from: " + durationUrl);
  ...
  fetch(durationUrl, {
    ...
    .then((data) => {
      if (data.success) {
        document.getElementById("footer-discount-percent").value =
  ...
  fetch("./api/coupon.php", {
    ...
    .then((text) => {
      console.log("[DEBUG] Load raw response:", text);
      alert("DEBUG: Raw response: " + text.substring(0, 200));
      try {
        const data = JSON.parse(text);
        console.log("[DEBUG] Load parsed data:", data);
        alert("DEBUG: Parsed - hours=" + data.hours + ", minutes=" + data.minutes + ", seconds=" + data.seconds);
        if (data.success) {
          document.getElementById("coupon-duration-hours").value = data.hours ?? 0;
          document.getElementById("coupon-duration-minutes").value = data.minutes ?? 0;
          document.getElementById("coupon-duration-seconds").value = data.seconds ?? 0;
          console.log("[DEBUG] Loaded values:", data.hours, data.minutes, data.seconds);
          alert("DEBUG: Input fields updated!");
        } else {
          console.error("[DEBUG] Load failed:", data.message);
          alert("DEBUG: Load failed - " + data.message);
        }
      } catch (e) {
        console.error("[DEBUG] JSON parse error:", e, "Response text:", text);
        alert("DEBUG: JSON parse error!");
      }
    })
    .catch((error) => {
...
// Save footer discount percentage and duration
function saveFooterDiscount() {
  const discount = document.getElementById("footer-discount-percent").value;
  const hours = parseInt(document.getElementById("coupon-duration-hours").value) || 0;
  const minutes = parseInt(document.getElementById("coupon-duration-minutes").value) || 0;
  const seconds = parseInt(document.getElementById("coupon-duration-seconds").value) || 0;

  const messageEl = document.getElementById("settings-message");

  // Validation: Không được phép cả 3 đều là 0
  const zeroCount = (hours === 0 ? 1 : 0) + (minutes === 0 ? 1 : 0) + (seconds === 0 ? 1 : 0);
  if (zeroCount >= 3) {
    messageEl.textContent = "Lỗi: Không được phép cả giờ, phút, giây đều bằng 0!";
    messageEl.style.color = "red";
    toast({
      title: "Lỗi",
      message: "Thời hạn không hợp lệ! Phải có ít nhất 1 giá trị lớn hơn 0.",
      type: "error",
      duration: 3000,
    });
    return;
  ...
  const saveBody = "action=update_coupon_duration&hours=" + hours + "&minutes=" + minutes + "&seconds=" + seconds;
  console.log("[DEBUG] Save request body:", saveBody);
  ...
  fetch("./api/coupon.php", {
    ...
    .then((text) => {
      console.log("[DEBUG] Save raw response:", text);
      try {
        const data = JSON.parse(text);
        console.log("[DEBUG] Save parsed data:", data);
        if (data.success) {
          messageEl.textContent = "Đã cập nhật cấu hình mã giảm giá thành công!";
          messageEl.style.color = "green";
          toast({
            title: "Thành công",
            message: "Đã cập nhật cấu hình mã giảm giá!",
            type: "success",
            duration: 3000,
          });
        } else {
          messageEl.textContent = data.message || "Lỗi cập nhật thời hạn!";
          messageEl.style.color = "red";
          toast({
            title: "Lỗi",
            message: data.message || "Không thể cập nhật thời hạn!",
            type: "error",
            duration: 3000,
          });
        }
      } catch (e) {
        console.error("[DEBUG] JSON parse error:", e, "Response:", text);
        messageEl.textContent = "Lỗi parse response!";
        messageEl.style.color = "red";
      }
    })
    .catch((error) => {
      console.error("[DEBUG] Fetch error:", error);
      messageEl.textContent = "Đã xảy ra lỗi kết nối!";
      messageEl.style.color = "red";
      toast({
        title: "Lỗi",
        message: "Đã xảy ra lỗi kết nối!",

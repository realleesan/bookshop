...
function showHomeProduct(products) {
    ...
    currentPage = 1; // ✅ reset về trang 1 mỗi khi load lại
...
function setupPagination(products) {
    ...
    // Luôn có trang đầu và cuối
    pagesToShow.add(1);
    pagesToShow.add(totalPages);

    // ✅ Trang hiện tại và 1 trang hai bên
    ...
    pagesToShow.forEach((page, index) => {
        if (index > 0 && page - pagesToShow[index - 1] > 1) {
            ...
            ellipsis.classList.add('page-nav-item', 'ellipsis');
        ...
        li.innerHTML = `<a href="javascript:;">${page}</a>`;

        li.addEventListener('click', () => {
            currentPage = page;
            displayList(products, perPage, currentPage);
            setupPagination(products); // ✅ render lại pagination với currentPage mới
...
function showCategory(category) {
    ...
    if (category === 'Phát triển bản thân' || category === 'Kinh doanh – tài chính' || category === 'Tiểu thuyết – truyện' || category === 'Học tập – kỹ năng') {
        rankingTabs.style.display = 'flex';
        showRanking('sold', null);
    } else {
...
function showRanking(type, event) {
    ...
    if (type === 'sold') {
        let rankingProducts = getRankingProducts();
        let productSearch = rankingProducts.filter(value =>
            value.category.toString().toUpperCase().includes(currentCategory.toUpperCase())
        );
        displayList(productSearch, perPage, currentPage);
        setupPagination(productSearch);
        return;
    ...
    let sortField = type === 'search' ? 'search_count' : 'like_count';

    fetch(`api/product_stats.php?category=${currentCategory}&sort=${sortField}`)
        .then(response => response.json())
        .then(products => {
            let result = products.length > 0 ? products : productAll.filter(value =>
                value.category.toString().toUpperCase().includes(currentCategory.toUpperCase())
            );
...
function getRankingProducts() {
    let soldMap = getBestSellingProducts();

    let products = productAll.map(product => {
        return {
            ...product,
            sold_count: soldMap[product.id] || 0
        };
    });
...
function getBestSellingProducts() {
    ...
    orderDetails.forEach(item => {
        let productId = item.product_id; // ✅ dùng đúng field
    ...
    fetch('api/coupon.php', {
        ...
        .then(data => {
            if (data.success) {
                ...
                let durationText = '';
                if (data.duration_hours > 0) durationText += data.duration_hours + ' giờ ';
                if (data.duration_minutes > 0) durationText += data.duration_minutes + ' phút ';
                if (data.duration_seconds > 0) durationText += data.duration_seconds + ' giây';
                if (durationText === '') durationText = 'vô thời hạn';

                toast({
                    title: 'Thành công',
                    message: 'Đăng ký nhận mã giảm giá thành công! Mã: ' + data.code + ' (Giảm ' + data.discount_percent + '%). Mã có hiệu lực trong ' + durationText.trim() + '.',
                    type: 'success',
                    duration: 8000 // 8 seconds for user to remember the code and duration
                });
                emailInput.value = '';
            } else {
                toast({

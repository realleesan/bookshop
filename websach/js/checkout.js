...
function getTotalWithShipping() {
  ...
  if (currentCheckoutOption === 2 && currentCheckoutProduct) {
    ...
    return (
      currentCheckoutProduct.soluong * currentCheckoutProduct.price +
      PHIVANCHUYEN
    );
  }
  // Cart checkout
...
function applyCouponDiscount() {
  ...
  let userEmail = currentUser && currentUser.email ? currentUser.email : "";
...
function thanhtoanpage(option, product) {
  ...
  tudenlay.addEventListener("click", () => {
    ...
    if (appliedCoupon && discountAmount > 0) {
      ...
      let discountAmountEl = document.getElementById("discount-amount");
      if (discountAmountEl)
        discountAmountEl.textContent = "-" + vnd(newDiscountAmount);
    } else {
      priceFinal.innerText = vnd(total);
    }
  });
...
async function xulyDathang(product) {
  ...
  if (tudenlay.classList.contains("active")) {
    ...
    if (!chinhanh1.checked && !chinhanh2.checked) {
      toast({
        title: "Chú ý",
        message: "Vui lòng chọn chi nhánh để lấy hàng!",
        type: "warning",
        duration: 4000,
      });
      return;
  ...
  if (tennguoinhan === "") {
    ...
    toast({
      title: "Lỗi",
      message: "Tên người nhận chỉ được chứa chữ cái!",
      type: "error",
      duration: 4000,
    });
    return;
